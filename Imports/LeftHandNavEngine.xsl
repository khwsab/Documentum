...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format">
	...
	<xsl:template match="Menu">
			...
			<!--googleoff: index-->
					...
					<tr>
						<td bgcolor="#CCCCCC"><img src="/images/spacer.gif" width="180" height="1" alt=""/></td>
					</tr>				
					...
					<![CDATA[
							...
							<tr>
								<td>
									...
									<tr>
										<td><img src="/images/spacer.gif" width="20" height="4" alt=""/></td>
										<td>				
											<table border="0" cellpadding="0" cellspacing="0">
					]]>
				</xsl:text>			
	...
	<xsl:template name="DisplayCategoryItem">
		...
		<!-- AM1819443 JZ 3/22/06 get language code -->
		<xsl:variable name="getlanguage" select="substring-before(substring-after($categoryURL, '-'), '/')"/>
		...
		<tr>
			<td class="{$categoryCellClass}">
				...
				<tr>
					...
					<!-- AM1819443 JZ 3/22/06 For Turkey-Turkish, chose Turkish translation -->
					<xsl:choose>
						<xsl:when test="$getlanguage='Turkish'">
						<td width="154" valign="top"><xsl:choose><xsl:when test="$newWindow='True'"><a href="{$categoryURL}" class="{$categoryLinkClass}" target="_new"><xsl:value-of select="translate($categoryName,'abcçdefgğhıijklmnoöprsştuüvyz','ABCÇDEFGĞHIİJKLMNOÖPRSŞTUÜVYZ')"/></a></xsl:when><xsl:otherwise><a href="{$categoryURL}" class="{$categoryLinkClass}"><xsl:value-of select="translate($categoryName,'abcçdefgğhıijklmnoöprsştuüvyz','ABCÇDEFGĞHIİJKLMNOÖPRSŞTUÜVYZ')"/></a></xsl:otherwise></xsl:choose></td>
						</xsl:when>
						<xsl:otherwise> 
						<td width="154" valign="top"><xsl:choose><xsl:when test="$newWindow='True'"><a href="{$categoryURL}" class="{$categoryLinkClass}" target="_new"><xsl:value-of select="translate($categoryName,'abcdefghijklmnopqrstuvwxyzçóíãáàõéêôúабвгдеёжзийклмнопрстуфхцчшщъыьэюяąęóćłńśźżäöåšžøüğşı','ABCDEFGHIJKLMNOPQRSTUVWXYZÇÓÍÃÁÀÕÉÊÔÚАБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯĄĘÓĆŁŃŚŹŻÄÖÅŠŽØÜĞŞI')"/></a></xsl:when><xsl:otherwise><a href="{$categoryURL}" class="{$categoryLinkClass}"><xsl:value-of select="translate($categoryName,'abcdefghijklmnopqrstuvwxyzçóíãáàõéêôúабвгдеёжзийклмнопрстуфхцчшщъыьэюяąęóćłńśźżäöåšžøüğşı','ABCDEFGHIJKLMNOPQRSTUVWXYZÇÓÍÃÁÀÕÉÊÔÚАБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯĄĘÓĆŁŃŚŹŻÄÖÅŠŽØÜĞŞI')"/></a></xsl:otherwise></xsl:choose></td>
						</xsl:otherwise>
					</xsl:choose> 
				</tr>
				</table>
