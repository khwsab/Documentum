<%@ Control Language="C#" AutoEventWireup="true" Inherits="xom.crown.nav.DynamicNavigation" %><script xmlns:asp="http://schemas.microsoft.com/ASPNET/20" type="text/javascript">
		...
		document.getElementById('lubricantselector').setAttribute("class","mnNavOn");
		document.getElementById('lubricantselector').className = "mnNavOn";
		
		addFallback(-1,document.getElementById('lubricantselector').href);
		document.getElementById('navTitle').innerHTML = document.getElementById('lubricantselector').innerHTML;
