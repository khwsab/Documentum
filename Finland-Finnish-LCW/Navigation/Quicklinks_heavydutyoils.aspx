
				<!-- googleoff: index -->
			<script type="text/javascript">quickEditForm("0915881d816d6276", "/Content/Quick_Links_Settings",true,"../forms/LCWQuickLinks","10px","820px","680px","Quick Links","","pencil.gif","");</script><div class="quicklinks">
    <div class="headerbar">
        <h3 class="nolink">pikalinkit</h3>
    </div>
    <ul>
        <li>
            <a title="" target="" onclick="trackClick(this);" href="/Finland-Finnish-LCW/heavydutyoils_mobil-delvac-product-selector.aspx" class="">mikä moottoriöljy</a>
        </li>
        <li>
            <a title="" target="" onclick="trackClick(this);" href="http://www.exxonmobil.fi" class="">tietoja meistä</a>
        </li>
        <li>
            <a title="" target="_blank" onclick="trackClick(this);" href="http://www.exxonmobil.com/pdssearch/search.aspx?chooseLanguage=fi&amp;CountryValue=Finland" class="">tuotetietosivut</a>
        </li>
        <li>
            <a target="_blank" onclick="trackClick(this);" href="http://www.msds.exxonmobil.com/psims/psims.aspx?language=FI&amp;brand=XOM&amp;lstext=Suomi&amp;languageselectedvalue=14">käyttöturvallisuustiedotteet</a>
        </li>
        <li>
            <a target="_blank" onclick="trackClick(this);" href="http://www.youtube.com/user/OfficialMobilDelvac">Mobil Delvac &ndash; YouTube</a>
        </li>
    </ul>
</div>
				<!-- googleon: index -->
			
