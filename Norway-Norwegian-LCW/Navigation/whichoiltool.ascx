<%@ Control Language="C#" AutoEventWireup="true" Inherits="xom.crown.nav.DynamicNavigation" %><script xmlns:asp="http://schemas.microsoft.com/ASPNET/20" type="text/javascript">
		...
		document.getElementById('whichoiltool').setAttribute("class","mnNavOn");
		document.getElementById('whichoiltool').className = "mnNavOn";
		
		addFallback(-1,document.getElementById('whichoiltool').href);
		document.getElementById('navTitle').innerHTML = document.getElementById('whichoiltool').innerHTML;
    ...
    <div class="subL">
        <div class="subR">
            ...
            <asp:panel visible="true" id="L1_1" Runat="server">
                <ul>
                    <li class="endpoint">
                        <asp:HyperLink runat="server" id="aL1_1" NavigateUrl="/Norway-Norwegian-LCW/oilselector.aspx">Mobil oljevelger</asp:HyperLink>
                    </li>
                </ul><% Response.Write((aL1_1.CssClass == "selected") ? "<script type=\"text/javascript\">addFallback(1,\"oilselector.aspx\")</script>" : ""); %></asp:panel>
            <div class="capBL">
                <div class="capBR"></div>
            </div>
        </div>
